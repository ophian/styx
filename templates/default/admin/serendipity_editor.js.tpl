...
(function(serendipity, $, undefined ) {
    ...
    // Toggle highlight elements in lists
    ...
    serendipity.closeCommentPopup = function() {
        {if $use_backendpopups OR isset($force_backendpopups.comments)}
            parent.self.close();
        {else}
...
$(function() {
    ...
    {if $use_backendpopups OR isset($force_backendpopups.categories)}
        if ($('#serendipityEntry').length > 0) {
    ...
    // Show tag selector
    {if $use_backendpopups OR isset($force_backendpopups.tags)}
        if ($('#serendipityEntry').length > 0) {
    ...
    {if $use_backendpopups OR (isset($force_backendpopups.links) AND $force_backendpopups.links)}
        if ($('#dashboard').length > 0) {
    ...
    {if {serendipity_getConfigVar key='uploadResize'} AND ({serendipity_getConfigVar key='maxImgWidth'} > 0 OR {serendipity_getConfigVar key='maxImgHeight'} > 0)}
