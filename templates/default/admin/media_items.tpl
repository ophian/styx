...
{if $media.manage}
                {$file.diskname|truncate:38:"&hellip;":true}
{else}
                {$file.diskname}
{/if}
{if NOT empty($file.orderkey)}
                : {$file.orderkey|escape}
{/if}
{if $file.hotlink}
...
{if isset($file.mimeicon) AND in_array($file.mediatype, ['audio', 'video', 'binary']) AND in_array($file.extension, ['mp3', 'm4a', 'wav', 'ogg', 'aif', 'aiff', 'flac', 'au', 'mp4', 'webm', 'ogv'])}
                ...
                {if NOT $media.manage AND $media.viewperm}<span class="media_player"><a href="{$link}"{else}<div{/if} class="media_mime_player_thumb">
                    <img src="{$img_src}" title="media object: {$img_title} as player" alt="{$img_alt}"><!-- media/manage -->
                {if NOT $media.manage AND $media.viewperm}</a></span> <span class="media_or">- OR -</span> <span class="media_link media_mime_player_thumb"><a href="{$link}&amp;mediaobject[link]=1" title="{$file.diskname}"><img src="{$img_src}" title="media object: {$img_title} as link" alt="{$img_alt}"><!-- media/manage --></a></span>{else}</div>{/if}
{else}
...
{if isset($file.sizeWebp) AND $file.sizeWebp > 0 AND $file.sizeWebp < $file.size}
                    <source type="image/webp" srcset="{if NOT isset($file.thumbSizeWebp) OR $file.thumbSizeWebp > 0}{$img_src_webp|default:''}{/if}">
{/if}
                    <img src="{$img_src}" class="ml_preview_img" title="{if NOT $media.enclose AND NOT $file.hotlink}{$CONST.THUMBNAIL_SHORT}: {/if}{$img_title}" alt="{$img_alt}"><!-- media/properties -->
                </picture>
...
{* singular media property page only - single file only - no file@key in need - but keep ported 0 key *}
{if NOT $media.enclose}
...
{if $file.property_saved === false}
            <span class="msg_notice"><span class="icon-info-circled"></span> {$CONST.MEDIA_PROP_STATUS}</span>
{/if}
