...
<div class="msg_success"><span class="icon-ok-circled" aria-hidden="true"></span> {$CONST.DONE}: {$CONST.SETTINGS_SAVED_AT|sprintf:"$timestamp"}</div>
{/if}
{if $has_config_groups > 0 OR (NOT empty($desc) AND empty($config_groups))}
...
{/if}
    ...
    <div class="plugin_info{if NOT empty($config_groups) OR (NOT empty($desc) AND empty($config_groups))} tgroup_info additional_info{/if}">
...
<div class="msg_notice"><span class="icon-attention-circled" aria-hidden="true"></span> {$CONST.NO_UPDATES}</div>
{else}
{foreach $pluggroups AS $pluggroup => $groupstack}
{if !empty($only_group) AND ($only_group AND $pluggroup != $only_group || empty($pluggroup))}{continue}{/if}
{if $only_group != 'UPGRADE'}
{foreach $groupnames AS $available_group => $available_name}
{if NOT empty($pluggroup) && $pluggroup === $available_group}

<h3>{$available_name}</h3>
{/if}
{/foreach}
{/if}
...
{if $plug.stackable}
            <span class="block_level stackable"><span class="icon-ok-circled" aria-hidden="true"></span>  {$CONST.ALREADY_INSTALLED} <span class="icon-plus" aria-hidden="true"></span> </span>
{/if}
...
{/if}
{if $updateAllMsg}
<span class="msg_success"><span class="icon-ok-circled" aria-hidden="true"></span> {$CONST.DONE}: {$CONST.PLUGIN_UPDATES_DONE}</span>
{/if}
