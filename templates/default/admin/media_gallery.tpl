...
{if ($media.page != 1 AND $media.page <= $media.pages) OR $media.page != $media.pages}
            ...
            <nav class="pagination">
                ...
                <ul class="clearfix">
                    <li class="first">{if $media.page > 1}<a class="button_link" href="{$media.linkFirst}&amp;serendipity[showGallery]=true" title="{$CONST.FIRST_PAGE}" aria-label="{$CONST.FIRST_PAGE}"><span class="icon-to-start" aria-hidden="true"></span></a>{/if}</li>
                    <li class="prev">{if $media.page != 1 AND $media.page <= $media.pages}<a class="button_link" href="{$media.linkPrevious}&amp;serendipity[showGallery]=true" title="{$CONST.PREVIOUS}" aria-label="{$CONST.PREVIOUS}"><span class="icon-left-dir" aria-hidden="true"></span></a>{else}<span class="sr-only">{$CONST.NO_ENTRIES_TO_PRINT}</span>{/if}</li>
                    {* Looks weird, but last will be at end by the CSS float:right *}
                    <li class="last">{if $media.page < $media.pages}<a class="button_link" href="{$media.linkLast}&amp;serendipity[showGallery]=true" title="{$CONST.LAST_PAGE}" aria-label="{$CONST.LAST_PAGE}"><span class="icon-to-end" aria-hidden="true"></span></a>{/if}</li>
                    <li class="next">{if $media.page != $media.pages}<a class="button_link" href="{$media.linkNext}&amp;serendipity[showGallery]=true" title="{$CONST.NEXT}" aria-label="{$CONST.NEXT}"><span class="icon-right-dir" aria-hidden="true"></span></a>{else}<span class="sr-only">{$CONST.NO_ENTRIES_TO_PRINT}</span>{/if}</li>
                </ul>
            </nav>
{/if}
