...
(function(serendipity, $, undefined ) {
    ...
    serendipity.confirmDialog = function(message, file, $el) {
        ...
        if (file) {
            dialog += '  <h2> {$CONST.DIALOG_DELETE_FILE_CONTINUE|sprintf:"' + file + '"}</h2>';
        }
        dialog += '  <div class="content"><p>' + message + '</p></div>';
        dialog += '  <div class="actions">';
        dialog += '    <button class="btn btn-primary btn-yes" type="button" role="button" aria-labelledby="ENTER-key"> {$CONST.YES} </button>';
        dialog += '    <button class="btn btn-default btn-cancel" type="button" role="button" aria-labelledby="ESC-key"> {$CONST.ABORT_NOW} </button> ';
        dialog += '    <button class="btn btn-second btn-no" type="button" role="button" aria-labelledby="SPACE-key"> {$CONST.NO} </button> ';
        dialog += '  </div>';
        dialog += '</div>';
    ...
    serendipity.addVariationsPerItem = function(id, fname) {
        var headline = '{$CONST.MEDIA_CREATEVARS}';
        var media_token_url = $('input[name*="serendipity[token]"]').val();
                    ...
                    items: {
                        type: 'inline',
                            src: $('<div id="addvar_msg">\
                                    <h4>'+headline+'</h4>\
                                    '+ jqXHR + '\
                                    <button id="addvar_ok" class="button_link state_submit" type="button"> {$CONST.GO} </button>\
                                    </div>')
                    },
                    type: 'inline',
                    midClick: true,
                    ...
                    items: {
                        type: 'inline',
                            src: $('<div id="addvar_msg">\
                                    ...
                                    <p>"Status: " + textStatus</p>\
                                    '+ errorThrown +'\
                                    <button id="addvar_error" class="button_link state_submit" type="button"> {$CONST.GO} </button>\
                                    </div>')
                    },
                    type: 'inline',
                    midClick: true,
...
$(function() {
    ...
    $('.media_delete').click(function(e) {
        e.preventDefault();
        var $el = $(this);
        serendipity.confirmDialog('{$CONST.DIALOG_DELETE_VARIATIONS_PERITEM}', $el.attr('data-filename'), $el);
    });
