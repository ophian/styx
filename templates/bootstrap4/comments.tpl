...
{foreach $comments AS $comment}
    <li id="c{$comment.id|default:0}" class="comment mb-4{if isset($entry) AND $entry.author == $comment.author AND $entry.email == $commentform_entry.email} serendipity_comment_author_self{/if} commentlevel_{if $comment.depth > 8}9{else}{$comment.depth}{/if}{if !isset($comment.id)} comment_preview{/if}">
        <ul class="comment_info plainList">
            <li class="d-inline-block"><svg class="icon-user" role="img" viewbox="0 0 1792 1792" width="1792" height="1792" aria-labelledby="title"><title id="title">{$CONST.POSTED_BY}</title><use xlink:href="{$serendipityHTTPPath}{$templatePath}{$template}/img/icons.svg#user"></use></svg>{if isset($comment.entryauthor) AND $comment.entryauthor == $comment.author AND isset($entry) AND $entry.email == $commentform_entry.email} <span class="pc-owner">Post author</span> {/if}{if $comment.url}<a href="{$comment.url}">{/if}{$comment.author|default:$CONST.ANONYMOUS}{if $comment.url}</a>{/if}</li>
