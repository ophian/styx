...
{elseif isset($content_message)}
    {if $content_message|strip == $content_message}{if $content_message == $CONST.URL_NOT_FOUND}<p class="alert alert-info">{else}<p>{/if}{$content_message}</p>{else}{$content_message}{/if}
{/if}
{$ENTRIES}
{$ARCHIVES}