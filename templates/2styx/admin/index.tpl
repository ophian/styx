...
{if NOT $admin_vars.no_footer}
    ...
    <footer id="meta">
        <p>{if $admin_vars.is_logged_in}{$admin_vars.version_info|replace:"Serendipity":"Serendipity Styx"}{else}{$admin_vars.version_info|regex_replace:"/([\d]+.?)+([-|beta|alpha]?)+([\s]?)+([a-z\.\+\d]+)/":''}{/if}</p>
    </footer>
{/if}
