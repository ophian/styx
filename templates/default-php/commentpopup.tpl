...
<?php if (!empty($GLOBALS['tpl']['is_comment_added'])): ?>
    ...
    <div class="popup_comments_message popup_comments_message_added"><?= sprintf(COMMENT_ADDED, "<a href=\"<?php if ($GLOBALS['tpl']['is_logged_in'] && isset($commentform_action)): ?><?= $GLOBALS['tpl']['commentform_action'] ?><?php endif; ?>#c<?= $_GET['last_insert_cid'] ?? '' ?>\">#<?= $_GET['last_insert_cid'] ?? '' ?></a> "); ?><?= $GLOBALS['tpl']['comment_string'][0] ?><a href="<?= $GLOBALS['tpl']['comment_url'] ?>"><?= $GLOBALS['tpl']['comment_string'][1] ?></a><?= $GLOBALS['tpl']['comment_string'][2] ?><a href="#" onclick="self.close()"><?= $GLOBALS['tpl']['comment_string'][3] ?></a><?= $GLOBALS['tpl']['comment_string'][4] ?></div>
