                                ...
                                <div class="{if $rightSidebarElements > 0}col-md-4{else}col-md-3{/if} col-xs-6">
                                    <div class="btn-group archive-button">
                                        {if $month.entry_count}
                                            <button type="button" class="btn btn-secondary btn-theme dropdown-toggle archive-button-months archive-button-months-entries" data-toggle="dropdown" aria-expanded="false">{$month.date|formatTime:"%b"}&nbsp;<span class="badge">{$month.entry_count}</span>&nbsp;&nbsp;<span class="caret"></span></button>
                                        ...
                                        {else}
                                            <button type="button" class="btn btn-secondary btn-theme disabled dropdown-toggle archive-button-months" data-toggle="dropdown" aria-expanded="false">{$month.date|formatTime:"%b"}</button>
                                        {/if}
                                    </div>
                                </div>
                            {/foreach}
