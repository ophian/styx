...
{foreach $comments AS $comment}
    <li id="c{$comment.id|default:0}" class="comment mb-4{if isset($entry) AND $entry.author == $comment.author AND $entry.email == $comment.clear_email} serendipity_comment_author_self{/if} {cycle values="odd,even"}{if $comment.depth > 8} commentlevel_9{else} commentlevel_{$comment.depth}{/if}">
...
{foreachelse}
    <li><span class="serendipity_msg_notice">{$CONST.NO_COMMENTS}</span></li>
{/foreach}
</ol>