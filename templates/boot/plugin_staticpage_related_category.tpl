{* hidden spacer break to fix startup indent for article *}
<!-- static page {$staticpage_pagetitle|makeFilename} -->
...
{if $staticpage_shownavi}
                    <ul class="pagination justify-content-between m-0 px-2 py-1">
                        <li class="page-item{if empty($staticpage_navigation.prev.link)} disabled{/if} text-start text-truncate pagenav_left">{if NOT empty($staticpage_navigation.prev.link)}<a class="btn btn-outline-secondary btn-sm" href="{$staticpage_navigation.prev.link}" title="prev">{$staticpage_navigation.prev.name|escape}</a>{else}<a class="page-link px-1 py-0 border-0 link-secondary"" href="#" tabindex="-1" aria-disabled="true">{$CONST.PREVIOUS}</a>{/if}</li>
...
{if $staticpage_adminlink AND $staticpage_adminlink.page_user}
                            (<svg class="mx-1" width="16" height="16" role="img" aria-labelledby="title"><title id="spcot">{$CONST.CREATED_ON|lower}</title><use xlink:href="#pencil-square"/></svg>{$staticpage_created_on|date_format:"%Y-%m-%d"})
{/if}
{else}
...
{if $staticpage_adminlink AND $staticpage_adminlink.page_user}
                            (<svg class="mx-1" width="16" height="16" role="img" aria-labelledby="title"><title id="splmdt">{$CONST.LAST_UPDATED|lower}</title><use xlink:href="#pencil-square"/></svg>{$staticpage_lastchange|date_format:"%Y-%m-%d"})
{/if}
{/if}
{/if}
{/if}
