...
{foreach $trackbacks AS $trackback}
        <li id="c{$trackback.id}" class="{$trackback.type|lower} {cycle values="tb-odd,tb-even"}{if $trackback.type|lower != 'trackback'} trackback{/if} d-flex justify-content-between">
{if $trackback.body == '' AND $trackback.type == 'PINGBACK'}
            <div><cite>{$trackback.author|default:$CONST.ANONYMOUS}</cite>: <time datetime="{$trackback.timestamp|serendipity_html5time}">{$trackback.timestamp|formatTime:$template_option.date_format}</time> <a href="{$trackback.url|strip_tags}">{$trackback.title}</a></div>
{/if}
...
{if $trackback.type == 'TRACKBACK'}
            <span class="serendipity_message_notice no-content">{$CONST.NO_ENTRIES_TO_PRINT}</span>
{/if}
{else}
            <details>
...
{if NOT empty($entry.is_entry_owner)}
                    <a href="{$trackback.link_delete}">{$CONST.TRACKBACK} {$CONST.DELETE}</a>
{/if}
                </div>
            </details>
{/if}
