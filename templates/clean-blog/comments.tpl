        ...
        <div id="div-comment-{$comment.id|default:0}" class="comment_any {cycle values="comment_odd,comment_even"} comment_author_{$comment.author|makeFilename}{if isset($entry) AND $entry.author == $comment.author AND $entry.email == $comment.clear_email} serendipity_comment_author_self{/if}">
            ...
            <div class="comment-list-item-body">
                ...
                <div class="comment-meta">
                    <a class="comment-source-trace btn btn-secondary btn-sm" href="#c{$comment.id|default:0}">#{$comment.trace}</a>
                    {if isset($entry) AND NOT empty($entry.is_entry_owner) AND NOT empty($comment.id)}
                        <a class="comment-source-ownerlink comment-reply-link btn btn-secondary btn-sm" href="{$comment.link_delete}" onclick="return confirm('{$CONST.COMMENT_DELETE_CONFIRM|sprintf:$comment.id:$comment.author}');" title="{$CONST.DELETE}"><i class="fa fa-lg fa-trash-o"></i><span class="sr-only"> {$CONST.DELETE}</span></a>
                    {/if}

                    {if isset($comment.id) AND NOT empty($entry.allow_comments) AND $comment.body != 'COMMENT_DELETED'}
                        <a class="comment-reply-link btn btn-secondary btn-sm" href="#serendipity_CommentForm" id="serendipity_reply_{$comment.id}" onclick="document.getElementById('serendipity_replyTo').value='{$comment.id}';{if NOT empty($comment_onchange)} {$comment_onchange|default:''}{/if}" title="{$CONST.REPLY}"><i class="fa fa-lg fa-reply"></i><span class="sr-only"> {$CONST.REPLY}</span></a>
