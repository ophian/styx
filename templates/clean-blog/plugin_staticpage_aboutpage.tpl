<article id="staticpage_{$staticpage_pagetitle|makeFilename}" class="serendipity_staticpage{if $staticpage_articleformat} post serendipity_entry{/if}">
    <header>
        ...
        <div class="staticpage_breadcrumbs">
            <a href="{$serendipityBaseURL}" title="{$CONST.HOMEPAGE}">{$template_option.home_link_text}</a> {if NOT empty($staticpage_navigation.crumbs)} &#187; {/if}
...
{/if}
    ...
    {/if}
    {if $staticpage_custom.show_author == 'true' OR $staticpage_custom.show_date == 'true' OR ($staticpage_adminlink AND $staticpage_adminlink.page_user)}

        <p class="post-meta">
            {if $staticpage_custom.show_author == 'true'}{$CONST.POSTED_BY} {$staticpage_author|escape}{/if}{if $staticpage_custom.show_date == 'true'}{if $staticpage_custom.show_author == 'true'} {$CONST.ON} {/if}<time datetime="{$staticpage_lastchange|serendipity_html5time}">{$staticpage_lastchange|formatTime:($template_option.date_format|default:$CONST.DATE_FORMAT_ENTRY)}</time>{/if}{if $staticpage_adminlink AND $staticpage_adminlink.page_user}{if $staticpage_custom.show_author == 'true' OR $staticpage_custom.show_date == 'true'}&nbsp;&nbsp;{/if}<a href="{$staticpage_adminlink.link_edit}" title="{$staticpage_adminlink.link_name|escape}"><button class="btn btn-secondary btn-sm"><i class="fa fa-lg fa-edit"></i><span class="sr-only">{$staticpage_adminlink.link_name|escape}</span></button></a>{/if}
        </p>
    {/if}
