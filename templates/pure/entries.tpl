...
{if NOT $is_single_entry AND isset($entry.properties.ep_no_frontpage) AND $entry.properties.ep_no_frontpage == true}{continue}{/if}
...
{* catch a staticpage startpage which has no $entries array set *}
{foreach $entries AS $dategroup}
            <section id="entries_dategroup" class="serendipity_Entry_Date">
{if NOT $is_single_entry AND NOT isset($dategroup.entries.{$dategroup@index}.properties.ep_no_frontpage)}
...
{if NOT empty($entry.categories)}{foreach $entry.categories AS $entry_category_pre}{if $entry_category_pre.category_icon}
                        <a class="category_icon" href="{$entry_category_pre.category_link}"><img class="serendipity_entryIcon" title="{$entry_category_pre.category_name|escape}{$entry_category_pre.category_description|emptyPrefix}" alt="{$entry_category_pre.category_name|escape}" src="{$entry_category_pre.category_icon}"></a>
{/if}{/foreach}{/if}
...
{if isset($view) AND $view == 'entry' AND NOT $use_popups}
                                <span>{$CONST.COMMENTS}: </span>
{/if}
...
{else}
                                <span>{$entry.freetag.tags.description} </span>
{/if}
...
{if isset($view) AND $view == 'entry'}
                                <span>{$CONST.ADMIN}: </span>
{/if}
                                <a href="{$entry.link_edit}">{if isset($view) AND $view == 'entry'}{$CONST.EDIT_ENTRY}{else}{$CONST.EDIT}{/if}</a>
                            </li>
{/if}
...
{if isset($view) AND $view == 'entry' || NOT empty($entry.properties.permalink)}
...
{/foreach}
{if $is_comment_added}
                    <p class="msg_notice">{$CONST.COMMENT_ADDED|sprintf:"<a href=\"{if $is_logged_in AND isset($commentform_action)}{$commentform_action}{/if}#c{$smarty.get.last_insert_cid|default:''}\">#{$smarty.get.last_insert_cid|default:''}</a> "}</p>
