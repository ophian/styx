            <article id="page_{$staticpage_pagetitle|makeFilename}" class="page serendipity_staticpage">
                <header>
...
{* remove the a:link to show as plain text *}{*
...
{if is_array($staticpage_childpages)}
                ...
                <div class="clearfix content staticpage_childpages">
                    <ul id="staticpage_childpages" class="page_children">
...
{if $staticpage_author}
                        {$CONST.POSTED_BY} {$staticpage_author|escape}
{/if}
{if $staticpage_lastchange}
                        {$CONST.ON} <time datetime="{$staticpage_lastchange|serendipity_html5time}">{$staticpage_lastchange|formatTime:($template_option.date_format|default:$CONST.DATE_FORMAT_ENTRY)}</time>
{/if}
