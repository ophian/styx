...
{if $plugin_calendar_head.minScroll le $plugin_calendar_head.month_date}
        <a href="{$plugin_calendar_head.uri_previous}" title="{$CONST.BACK}">&larr;<span> {$CONST.BACK}</span></a>
{/if}
        </td>
        <td class="month">
...
{if $plugin_calendar_head.maxScroll ge $plugin_calendar_head.month_date}
        <a href="{$plugin_calendar_head.uri_next}" title="{$CONST.FORWARD}"><span>{$CONST.FORWARD} </span>&rarr;</a>
{/if}
        </td>
    </tr>
</tfoot>
