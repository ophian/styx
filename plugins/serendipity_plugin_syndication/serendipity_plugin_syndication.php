...
class serendipity_plugin_syndication extends serendipity_plugin
{
    ...
    private const SYNDICATION_PLUGIN_OUTDATED_SERVICES_DESC = "Explanation: In the old days, Googles Feedburner Service was the central collector for feeds, a provider for managing web feeds. Long ago, Google moved this to the “Google graveyard of dead projects”, so said, deprecated the API and shut off certain inbound services like AdSense for Feeds, but left it alive for registered users. Current state: You don't need it nowadays.<br>The other, Subtome, was a project to gather feeds all in-one, in short a subscribing application, but didn't made it to become very famous. One of the key goals of the button was to hide “RSS” altogether. Its relevance for feeds is near to nothing today. On the longer run both services will get nuked from the syndication plugin. There are so many valid subscribing tools and feedreaders that providing the xml buttons and feeds is the purest implementation we should give.";
