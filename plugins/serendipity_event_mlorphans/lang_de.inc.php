...
@define('MLORPHAN_MTASK_MAIN_PATTERN_ID_ERROR', 'Bitte korrigieren Sie umgehend: Eintrag: %d  [<em>%s</em> textarea Feld] auf ein <em>voraussichtlich</em> <b>falsch</b> genutztes "<code>&lt;-- s9ymdb: %d --&gt;</code>" tag zu einem Bild src Pfad von "<em>%s</em>" und starten danach dieses Skript erneut. Für die gesuchte Bild-ID, suchen Sie in ihrer Mediathek, zB. im GET String ( &serendipity[fid]=XX ) der Medien Eigenschaften, nach der zugehörenden ID, oder durchforsten Sie das Waisen-Array in der untenstehenden Ausklappbox "view array" nach "<em>%s</em>", um die korrekte Bild ID [id] zu erfahren.');
...
@define('MLORPHAN_MTASK_MAIN_PATTERN_NAME_WARNING', 'ACHTUNG: Ein Bild darf grundsätzlich keinen Punkt im eigentlichen Bild Namen haben! Frühere Serendipity Versionen aber haben dies beim Upload nicht korrekt gecheckt! Vermeiden Sie solche Vorkommen in ihren Beständen, sonst können Mediatheks-Aktionen wie Verschiebungen / Umbenennungen / etc. zu schwer reparierbaren Folgen führen!');
