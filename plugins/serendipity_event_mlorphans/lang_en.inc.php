...
@define('MLORPHAN_MTASK_LAST_ACTION_NOTE', '<b>Take care:</b> <span>There <b>may</b> still be some of these images used in your entries <b>without</b> a <code>&lt;-- s9ymdb:N --&gt;</code> MediaLibrary tag. If being unsure about that, do not delete here or better re-/add the correct tag IDs in these entries and run this script again. (Recommended!)</span><br>
    <span>This orphan manager task also does <b>not</b> care about entryproperties (plugin) database cached entries. If you had to correct existing, but false set image s9ymdb:IDs (see possible error messages on top) in articles manually, this also applies to entryproperties stored cache entries. If you need and use that (but this shouldn\'t be necessarily a need these days), run the maintenance entryproperties build cache again afterwards, which may lead to other issues such as NL2BR and line breaks in very old articles.</span>');
...
@define('MLORPHAN_MTASK_MULTIPOST_AUTOFIX', 'For cases where matches¹ and fixes were not being greedy enough, run this all from the beginning again (and then again) as long there are thrown red errors with possible replacement string parts, please. You will notice that errors and replacements are slowly getting less (reading upwards) until at the end they all are captured. Depending on the initial situation in your entries, this repetition may be necessary many times.<br>[¹ <em>deliberately un-greedy</em> ]');
