            ...
            foreach ($sql AS $key => $row) {
                ...
                if (!$cssbreak) {
                    ...
                    if (function_exists('mb_strimwidth') && function_exists('mb_strrpos') && function_exists('mb_substr')) {
                        $pos = 0;
                        $parts = array();
                        $comment_len = mb_strlen($comment, LANG_CHARSET);
                        ...
                        while ($pos < $comment_len) {
                            ...
                            if (($comment_len - $pos) > $wordwrap) {
                                ...
                                $part = ( $spacepos > 0 ) ? mb_substr($comment, $pos, $spacepos, LANG_CHARSET) : mb_strimwidth($comment, $pos, $wordwrap, '', LANG_CHARSET);
                            } else {
                                # wrap "hard", i.e. truncate words that are too long
                                $part = mb_substr($comment, $pos, $wordwrap, LANG_CHARSET);
                            }
                            # forward the pointer
                            $pos += mb_strlen($part, LANG_CHARSET);
...
?>
                    ...
                    <div class="plugin_comment_wrap mb-2">
                        <?=$spawn ?>
