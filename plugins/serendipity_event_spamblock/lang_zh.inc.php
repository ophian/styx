...
@define('PLUGIN_EVENT_SPAMBLOCK_CAPTCHAS_DESC', '这将强制要求用户输入在特制的图片中显示的一个随机字符串。这可以防止机器人自动向你的博客发送回复信息等。需要注意的是，视力不好的访客可能很难看清这些验证码上的字符串。 To avoid having to use visible Captchas at all, try out the extending Spamblock Bee plugin.');
...
@define('PLUGIN_EVENT_SPAMBLOCK_LOGTYPE_DESC', '被拒绝的回复可以被记录在数据库或文本文件中。日誌應不時以手動方式維護，以防止系統因日誌大小增加而負荷過重，並找出錯誤的拒絕。資料庫日誌可透過 Maintenance 中的維護任務清理。');
