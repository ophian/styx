                ...
                case 'frontend_comment':
                    ...
                    if ($_show_captcha) {
                        ...
                        if (!isset($serendipity['POST']['preview']) || strtolower($serendipity['POST']['captcha'] ?? '') != @strtolower($_SESSION['spamblock']['captcha'] ?? '')) {
                            ...
                            echo '                                    <input id="'.$selector_id.'" class="input_textbox input_captcha" type="text" size="5" name="serendipity[captcha]" value="" />';
            ...
            <div class="serendipity_cpmdiff" style="margin-top: .5em;">
                ...
                <form id="maintenance_cleanspam_multi" enctype="multipart/form-data" action="<?php echo $serendipity['serendipityHTTPPath'] . (($serendipity['rewrite'] == 'rewrite') ? '' : 'index.php?/') ?>plugin/cleanspam/multi" method="post">
                    <select id="cleanspam_access_multi_reasons" class="additional_info" name="serendipity[cleanspam][multi_reasons][]" multiple="multiple">
                        ...
                        <option value="art">LIKE "<?php echo PLUGIN_EVENT_SPAMBLOCK_REASON_TITLE; ?>"</option>
...
/*
                case 'backend_sidebar_admin': // this is section: settings - append [ disabled ,for the move into comments ]
                    echo '<li><a href="serendipity_admin.php?serendipity[adminModule]=plugins&amp;serendipity[plugin_to_conf]=' . $this->instance . '">' . PLUGIN_EVENT_SPAMBLOCK_TITLE . "</a></div></li>\n";
                    break;
*/
                default:
                    return false;
    ...
    function log($logfile, $id, $switch, $reason, $comment)
    {
        ...
        switch($method) {
            case 'file':
                ...
                // Fix log file(!) messaging for htmlcomments && array to string
                if (!empty($comment['comment'])) {
                    $comment['comment'] = strip_tags(json_encode($comment['comment']));
                }
            ...
            if ($part[1] == 'multi') {
                ...
                if (is_array($multir) & !empty($multir)) {
                    foreach($multir AS $p) {
                        ...
                        if ($p == 'art') {
                            @serendipity_db_query("DELETE FROM {$serendipity['dbPrefix']}spamblocklog WHERE type LIKE 'REJECTED' AND reason='".PLUGIN_EVENT_SPAMBLOCK_REASON_TITLE."'"); // since already translated variously.., we have to use the constant
                            @serendipity_db_query("DELETE FROM {$serendipity['dbPrefix']}spamblocklog WHERE type LIKE 'MODERATE' AND reason='".PLUGIN_EVENT_SPAMBLOCK_REASON_TITLE."'"); // since already translated variously.., we have to use the constant
                            $sbldone = true;
                        }
                        if ($p == 'amx') {
                            @serendipity_db_query("DELETE FROM {$serendipity['dbPrefix']}spamblocklog WHERE type LIKE 'REJECTED' AND (reason='".PLUGIN_EVENT_SPAMBLOCK_REASON_FORCEMODERATION."' OR reason='Auto-moderation after X days')"); // Auto-moderation after X days, we use the constant and <en> lang
