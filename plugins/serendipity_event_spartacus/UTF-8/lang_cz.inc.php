...
@define('PLUGIN_EVENT_SPARTACUS_FETCHERROR', 'URL %s nelze otevřít. Server github nebo mirror možná neběží - omlouváme se, zkuste to později. Try to reload (F5) the page first.');
...
@define('PLUGIN_EVENT_SPARTACUS_REPOSITORY_ERROR', '(On-line úložiště vrátilo chybu %s.)');
@define('PLUGIN_EVENT_SPARTACUS_HEALTHCHECK', 'Nebylo možno získat data z on-line úložiště modulu Spartacus. Zkouška dostupnosti úložiště na adrese %s.');
@define('PLUGIN_EVENT_SPARTACUS_HEALTHERROR', 'Diagnstická stránka modulu Spartacus vrátilo chybu (HTTP kód %s). To může znamenat, že je dignostická stránka mimo provoz. Zkuste to prosím později.');
@define('PLUGIN_EVENT_SPARTACUS_HEALTHLINK', '<a target="_blank" rel="noopener" href="%s">Klikněte zde pro zobrazení diagnstické stránky modulu Spartacus</a> a zjistěte, zda odpovídá.');
@define('PLUGIN_EVENT_SPARTACUS_HEALTHBLOCKED', 'SPARTACUS se pokusil připojit ke Googlu, ale neúspěšně (chyba č.%d: %s).<br/>Váš server blokuje odchozí spojení. SPARTACUS nebude fungovat, protože se nemůže připojit do svého online úložiště. <b>Kontaktujte prosím svého poskytovatele webhostingu a požádejte ho o povolení odchozích připojení k jiným webovým stránkám.</b> Zásuvné moduly lze instalovat z lokálních adresářů. Jednoduše stáhněte nejnovější verzi pluginu z <a href="http://spartacus.s9y.org">SPARTACUS origin web úložiště</a>, rozbalte ho a například pomocí FTP nahrajte do adresáře /plugins/ ve Vaší instalaci Serendipity. For Styx link to the <a href="https://ophian.github.io/plugins/">Styx Spartacus website</a>.');
@define('PLUGIN_EVENT_SPARTACUS_HEALTHDOWN', 'Modulu SPARTACUS se podařilo spojit s Googlem, ale nemůže se spojit s on-line úložištěm modulu SPARTACUS. Je možné, že Váš server blokuje některá odchozí spojení, nebo je on-line úložiště mimo provoz. Spojte se se svým poskytovatelem webového serveru, abyste se ujistili, že nemáte blokována odchozí spojení. <b>SPARTACUS nebude fungovat, protože nemůže kontaktovat on-line úložiště.</b>');
...
#@define('PLUGIN_EVENT_SPARTACUS_CUSTOMMIRROR_DESC', 'Tuto volbu používejte jen pokud opravdu víte, co děláte, jste-li zkušený uživatel. Pokud jsou přednastavené mirror servery vypnuté nebo nepracují správně, Serendipity se pokusí připojit k Vámi zadaným mirror serverům (třeba můžete zadat https://mirror.org/serendipity/). Tyto servery musí umět zpracovávat XML soubory na úrovni URL adresy a musí obsahovat podadresáře "additional_plugins" a "additional_themes". Zadávejte pouze mirror servery, kterým věříte a které plně zrcadlí všechny soubory CVS úložiště Serendipity. Lze zadat více zrcadel, oddělených "|".');
