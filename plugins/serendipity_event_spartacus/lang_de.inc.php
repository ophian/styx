...
@define('PLUGIN_EVENT_SPARTACUS_HEALTHBLOCKED', 'SPARTACUS konnte keine Test-Verbindung zu Google herstellen (Fehler %d: %s).<br>Ihr Server blockiert vermutlich ausgehende Verbindungen. SPARTACUS kann so nicht ausgeführt werden, da so auf keine der SPARTACUS-Quellen zugegriffen werden kann. <b>Bitte kontaktieren Sie ihren Web-Provider und bitten ihn, ausgehende HTTP-Verbindungen zuzulassen.</b> Plugins können nach wie vor auch lokal installiert werden. Laden Sie dazu einfach ein Plugin von <a href="http://spartacus.s9y.org">der originalen SPARTACUS Webseite</a> herunter, entpacken es und laden es in ihr Serendipity "plugins"-Verzeichnis hoch. Beizeiten wird Styx ein eigenes Web-Repository anbieten.');
...
@define('PLUGIN_EVENT_SPARTACUS_CUSTOMMIRROR', 'STYX Repository oder eigene Mirror-Quelle');
@define('PLUGIN_EVENT_SPARTACUS_CUSTOMMIRROR_DESC', 'Diese Option ist normalerweise nur für Experten gedacht. Falls keiner der voreingestellten Mirror-Server aufgrund von Downtime oder Problemen verfügbar ist, kann hier ein eigener Server-Name wie "https://mirror.org/serendipity/" eingetragen werden. Dort müssten die XML-Dateien für Spartacus im Verzeichnis liegen, und Unterverzeichnisse wie "additional_plugins" und "additional_themes" existieren. Geben Sie hier nur Mirrors ein, denen Sie voll vertrauen, und auf denen eine Kopie des Repositories gespeichert ist. Mehrere Mirrors können mit "|" getrennt eingegeben werden.');
@define('PLUGIN_EVENT_SPARTACUS_CUSTOMMIRROR_DESC_ADD', 'ACHTUNG: Die Warnung gilt daher nur noch begrenzt und NICHT für Styx. Die hier per default voreingestellte URL ist auf das Styx "additional_plugins" Verzeichnis limitiert. Zusätzliche THEMES werden weiterhin von den oben eingestellten Spiegelservern gezogen. Dies kann sich in Zukunft ändern. Es ist keineswegs empfehlenswert diesen eigenständigen Styx Plugin-Mirror NICHT zu nutzen, da zu viele Änderungen und Fixes bereits eingeflossen sind und das Styx Plugin repository auf Styx abgestimmt und dem Original schon weit voraus ist! Möchten Sie grundsätzlich nur die obigen Spiegelserver nutzen, müssen Sie vor jedem Konfigurationssubmit die hier eingestellte URL per Hand löschen.');
...
@define('PLUGIN_EVENT_SPARTACUS_CSPRNG', ' (Bei Aufruf dieser Seite erneuerter String: "%s%s". Kopieren Sie diese "gehashte" Zeichenkette (ohne ""), um sie zusammen mit dem serendipity_event_plugup Plugin für einen "Dashboard"-Check-Plugin-Update Hinweis zu nutzen. Erlauben Sie dafür obige Fernabfrage Option. Solch eine Seite enthält, im Gegensatz zu der der originalen "spartacus_remote" Zeichenkette, keine nennenswert zu schützende Information, außer der, dass ganz allgemein Plugin Updates vorliegen.)');
