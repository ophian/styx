...
@define('CREATE_THUMBS', 'Reconstruir las miniaturas');
...
@define('HERE_YOU_CAN_ENTER_BLAHBLAH', 'Aquí puede ajustar el tamaño de la imagen. Si desea cambiar el tamaño de la imagen proporcionalmente, solo tiene que introducir un valor en uno de los dos cuadros y pulsar la tecla TAB o el botón del ratón. Se calculará automáticamente el nuevo tamaño para que las proporciones de la imagen no se vean alteradas. <p><b>NOTA:</b> Esta no es una herramienta de edición de imágenes de alta gama, ajustada para las necesidades de una imagen específica. <br>Cada escala devuelve una pérdida de calidad de imagen más o menos considerable en comparación con el archivo de entrada original. ¡Y esto aumenta con cada nueva escala!</p>');
...
@define('COMMENT_ADDED', 'Tu comentario %sha sido añadido correctamente.');
...
@define('CANT_EXECUTE_BINARY', 'No se puede ejecutar la ruta binaria %s (no es obligatorio cuando se utiliza el módulo de extensión).');
...
@define('SERENDIPITY_UPGRADER_YOU_HAVE_IGNORED', 'Has ignorado el paso de actualización de Serendipity, asegúrate que tu base de datos está correctamente instalada y que las funciones planeadas son ejecutadas. Después de haber terminado su trabajo razonado para la retención, esto se puede hacer automáticamente con sólo restablecer la versión en su archivo serendipity_config_local.inc y ejecutar esta página de actualización de nuevo a través de su página backend.');
...
@define('INSTALL_DBPASS_DESC', 'La contraseña para el usuario. Si ya está configurado, sólo existe en el archivo de acceso "serendipity_config_local.inc", o puede configurarse aquí.');
...
@define('INSTALL_WYSIWYG', 'Utilizar el editor RichText (WYSIWYG)');
@define('INSTALL_WYSIWYG_DESC', 'Este editor se utilizará en campos de texto comunes como formularios de entrada, páginas estáticas, comentarios (que tiene una opción global adicional) y algunos otros lugares de plugins; Almacena el marcado HTML directamente.');
...
@define('PGSQL_REQUIRED', 'Debes tener la extensión PostgreSQL para poder llevar a cabo esta acción.');
@define('COULDNT_CONNECT', 'No se puede conectar al gestor de base de datos MySQL: %s.');
@define('PGSQL_COULDNT_CONNECT', 'No se puede conectar al gestor de base de datos PostgreSQL: %s.');
...
@define('DELETE_DIRECTORY_DESC', 'Está a punto de eliminar un directorio<sup>(¹)</sup> que puede contener archivos multimedia y elementos ya utilizados en sus entradas. Asegúrese!<br><sup>[¹]</sup> Incluyendo el subdirectorio oculto con archivos de variación (WebP/AVIF).');
...
@define('ERROR_XSRF', 'Su navegador no ha enviado una cadena HTTP-Referrer válida. En el caso del remitente, puede deberse a una configuración incorrecta del navegador/proxy o a una falsificación de petición en sitios cruzados (XSRF) dirigida a usted. De lo contrario, este mensaje sólo puede indicar que su pestaña ha estado abierta durante demasiado tiempo y su token de edición actual ha caducado. La acción solicitada no ha podido completarse.');
