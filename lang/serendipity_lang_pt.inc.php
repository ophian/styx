...
@define('DATE_LOCALES', 'pt_BR.UTF-8, pt_BR.UTF8, pt_BR, portuguese brazilian, pt-br, pt');
@define('DATE_FORMAT_ENTRY', '%A, %e de %B de %Y');
@define('DATE_FORMAT_SHORT', '%d/%m/%Y %H:%M');
@define('WYSIWYG_LANG', 'pt_BR');
@define('TINYMCE_LANG', 'pt');
...
@define('IMAGESYNC_WARNING', 'ATENÇÃO:<br>Isso pode levar muito tempo se existirem muitas imagens sem miniaturas. Particularly with migrations of old blogs, further preliminary considerations and knowledge are necessary! Read about it on <a href="https://ophian.github.io/hc/en/media-migration-tasks.html" target="_new">this help page</a>, carefully.');
...
@define('HERE_YOU_CAN_ENTER_BLAHBLAH', 'Aqui, você pode ajustar o tamanho da imagem. Se quiser redimensionar a imagem proporcionalmente, basta inserir um valor em uma das duas caixas e pressionar a tecla TAB ou o botão do mouse — ele calculará automaticamente o novo tamanho para que as proporções da imagem não sejam alteradas. <p><b>ATENÇÃO:</b> Esta não é uma ferramenta de redimensionamento de imagens de alta qualidade, ajustada para as necessidades de uma imagem específica.<br>Cada escala resulta em uma perda mais ou menos significativa da qualidade da imagem em comparação com o arquivo original. E isso aumenta a cada nova escala!</p>');
...
@define('SUBSCRIBE_TO_BLOG', 'Inscreva-se neste blog');
...
@define('COMMENT_ADDED', 'Seu comentário %sfoi adicionado corretamente.');
...
@define('FILE_NOT_FOUND', 'Não foi possível localizar o arquivo intitulado %s, será que foi excluído?');
...
@define('DATE_INVALID', 'Atenção: A data que você especificou é inválida. Ela deve ser fornecida no formato inglês AAAA-MM-DD HH:MM.');
...
@define('DO_MARKUP_DESCRIPTION', 'Aplicar (plugin) transformações de marcação no texto (smilies, bbcode, s9y marcação de atalhos, markdown, etc.). Se desabilitado, o conteúdo é renderizado 1:1 e qualquer formatação HTML é preservada. Se esta opção for ativada, outros plugins podem modificar o conteúdo do nugget.');
...
@define('SERENDIPITY_UPGRADER_YOU_HAVE_IGNORED', 'Você ignorou a fase de atualização do Serendipity, certifique-se de que o banco de dados está instalado corretamente e que as funções necessárias foram executadas. Depois de concluir seu trabalho fundamentado de retenção, isso pode ser feito automaticamente, bastando redefinir a versão em seu arquivo serendipity_config_local.inc e executar essa página de atualização novamente por meio de sua página de back-end.');
...
@define('INSTALL_DBPREFIX_DESC', 'Prefixo utilizado para nomear as tabelas, ex.: styx_');
...
@define('INSTALL_WYSIWYG', 'Usar o editor RichText (WYSIWYG)');
@define('INSTALL_WYSIWYG_DESC', 'Esse editor será usado em campos de área de texto comuns, como formulários de entrada, páginas estáticas, comentários (que têm uma configuração de opção global adicional) e alguns outros locais de plug-in; ele armazena a marcação HTML diretamente.');
...
@define('INSTALL_IMAGEMAGICK_DESC', 'Você tem a biblioteca binária ImageMagick ou o módulo Imagick instalado e deseja usá-los para redimensionar imagens?');
...
@define('DELETE_THUMBNAIL', 'Excluir a miniatura da imagem intitulada %s');
@define('DELETE_FILE', 'Excluir o campo entitulado %s');
@define('ABOUT_TO_DELETE_FILE', 'Você está prestes a excluir <b>%s</b><br>Se estiver utilizando esse arquivo em algum dos seus artigos, vai resultar em links ou imagens perdidas<br>Deseja realmente prosseguir com a exclusão?');
...
@define('USERCONF_USERLEVEL_DESC', 'Este nível será usado para determinar que tipo de acesso os usuários terão em seu blog. Os privilégios dos usuários são tratados pelos membros do grupo!');
...
@define('CONVERT_HTMLENTITIES', 'Tentar auto-converter as entidades HTML?'); // Translate again
...
@define('DELETE_HOTLINK_FILE', 'Apagar o entrelaçamento do arquivo de nome %s');
...
@define('BACKEND_TITLE_FOR_NUGGET', 'Aqui você pode definir um texto personalizado que aparecerá na tela de configuração de plugin, junto com a descrição do plugin. Se você possui múltiplos stacked plugins / HTML nuggets com nome em branco, isto irá ajudar a separar uns dos outros.');//verify
...
@define('PLUGIN_GROUP_BACKEND_MAINTAIN', 'Página posterior: Maintenance');
@define('PLUGIN_GROUP_BACKEND_DASHBOARD', 'Página posterior: Dashboard');
...
@define('COMMENT_NOT_ADDED', 'Seu comentário não pôde ser adicionado, porque os comentários para esta entrada ou foram desativados, você inseriu dados inválidos, ou seu comentário foi pego por medidas anti-spam.');
