...
@define('THUMB_CREATED_DONE', 'Náhled %s je vytvořen.<br>Hotovo.'); // ADD: and of all variations
...
@define('HERE_YOU_CAN_ENTER_BLAHBLAH', 'Zde můžete upravit velikost obrázku. Pokud chcete změnit velikost obrázku proporcionálně, stačí zadat hodnotu do jednoho ze dvou polí a stisknout klávesu TAB nebo tlačítko myši – automaticky se vypočítá nová velikost, aby se neporušily proporce obrázku. <p><b>UPOZORNĚNÍ:</b> Nejedná se o špičkový nástroj pro úpravu velikosti obrázků, který je přizpůsoben potřebám konkrétního obrázku. <br>Každé změně velikosti odpovídá více či méně zvýšená ztráta kvality obrázku ve srovnání s původním vstupním souborem. A s každou další změnou velikosti se tato ztráta zvyšuje!</p>');
...
@define('PGSQL_COULDNT_CONNECT', 'Nebylo možné se spojit s databází PostgreSQL: %s.');
...
@define('CANT_EXECUTE_BINARY', 'Nelze spustit binární cestu %s (není povinné při použití rozšiřujícího modulu)');
...
@define('FILE_NOT_FOUND', 'Nelze nalézt soubor s názvem %s, možná je opravdu vymazán?');
...
@define('DO_MARKUP_DESCRIPTION', 'Použijte (zásuvný modul) transformace značek na text (smajlíky, bbcode, značky zkratek s9y, markdown atd.). Pokud je vypnuto, obsah se vykreslí 1:1 a veškeré formátování HTML zůstane zachováno. Pokud je tato volba aktivována, mohou ostatní zásuvné moduly obsah samorostu upravovat.');
...
@define('SERENDIPITY_UPGRADER_YOU_HAVE_IGNORED', 'Vynechal jste stupeň upgrade Serendipity, ujistěte se, že databáze je správně nainstalována a plánované funkce jsou spuštěny. Poté, co jste dokončili zdůvodněnou práci pro zadržení, lze toto provést automaticky pouhým resetováním verze v souboru serendipity_config_local.inc a opětovným spuštěním této aktualizační stránky prostřednictvím vaší backendové stránky.');
...
@define('INSTALL_WYSIWYG', 'Použití editoru RichText (WYSIWYG)');
@define('INSTALL_WYSIWYG_DESC', 'Tento editor se používá v běžných polích textarea, jako jsou vstupní formuláře, statické stránky, komentáře (které mají další nastavení globální volby) a v některých dalších zásuvných modulech; Ukládá přímo značky HTML.');
...
@define('DELETE_FILE_FAIL', 'Nezdařilo se vymazat soubor %s');
...
@define('FORCE_DELETE', 'Odstranění VŠECH souborů v tomto adresáři, včetně podadresářů a souborů, které (zatím) nejsou známy knihovně MediaLibrary');
...
@define('ERROR_XSRF', 'Váš prohlížeč neodeslal platný řetězec HTTP-Referrer. V případě odkazujícího serveru to může být způsobeno buď špatně nakonfigurovaným prohlížečem/proxy serverem, nebo útokem XSRF (Cross Site Request Forgery) zaměřeným na vás. V opačném případě může tato zpráva pouze indikovat, že vaše karta byla otevřena příliš dlouho a platnost aktuálního editačního tokenu vypršela. Vámi požadovanou akci nebylo možné dokončit.');
...
@define('RSS_IMPORT_WPXRSS', 'WordPress eXtended RSS import může zabírat hodně paměti RAM');
...
@define('NOT_WRITABLE_SPARTACUS', 'Vyžadováno pro možnost vzdáleného stahování zásuvných modulů, která je ve výchozím nastavení aktivní prostřednictvím zásuvného modulu Spartacus!');
