...
@define('HERE_YOU_CAN_ENTER_BLAHBLAH', 'ここでは画像サイズを調整できます。画像を比率を保ったままリサイズしたい場合は、2つのボックスのいずれかに値を入力し、TABキーまたはマウスキーを押してください。画像の比率が崩れないよう、新しいサイズが自動的に計算されます。<p><b>ご注意：</b>これは特定の画像ニーズに合わせて微調整された、ハイエンドな画像編集ツールのサイズ変更機能ではありません。<br>どの拡大縮小でも、元の入力ファイルと比較して多少なりとも画質劣化が生じます。そして、拡大縮小を繰り返すほどこの劣化は進行します！</p>');
...
@define('SERENDIPITY_UPGRADER_YOU_HAVE_IGNORED', 'Serendipity のアップグレード ステージを無視しました。正しいデータベースが確実にインストールされ、スケジュール機能を実行します。 serendipity_config_local.incファイルのバージョンをリセットし、バックエンドページからこのアップグレードページを再度実行するだけです。');
...
@define('INSTALL_IMAGEDIM_WIDTH', '幅');
@define('INSTALL_IMAGEDIM_HEIGHT', '高さ');
...
@define('SUCCESS', '成功');
@define('POWERED_BY_SHOW_TEXT', 'テキストとして "%s" を表示する');
@define('POWERED_BY_SHOW_TEXT_DESC', 'テキストとして "Serendipity Styx Weblog" を表示するでしょう。');
@define('POWERED_BY_SHOW_IMAGE', '"%s" とロゴを表示');
@define('POWERED_BY_SHOW_IMAGE_DESC', '%s ロゴを表示します。');
...
@define('PLUGIN_GROUP_BACKEND_MAINTAIN', 'バックエンド: Maintenance');
@define('PLUGIN_GROUP_BACKEND_DASHBOARD', 'バックエンド: Dashboard');
...
@define('PERMISSION_ADMINIMPORT', 'adminImport: エントリーのインポート');
...
@define('PERMISSION_ADMINUSERSEDITUSERLEVEL', 'adminUsersEditUserlevel: ユーザー レベルの変更');
...
@define('PERMISSION_ADMINUSERSGROUPS', 'adminUsersGroups: ユーザー グループの管理');
...
@define('PERMISSION_ADMINIMAGES', 'adminImages: メディア ファイルの管理');
@define('PERMISSION_ADMINIMAGESDIRECTORIES', 'adminImagesDirectories: メディアディレクトリーの管理');
@define('PERMISSION_ADMINIMAGESADD', 'adminImagesAdd: 新規メディア ファイルの追加');
@define('PERMISSION_ADMINIMAGESDELETE', 'adminImagesDelete: メディア ファイルの削除');
...
@define('STYLE_OPTIONS_NONE', 'このテーマ、スタイルは指定オプションがありません。To see how your template can specify options, read the Technical Documentation on "https://ophian.github.io/hc/en/templating.html#docs-theme-options" about "Configuration of Theme options".');
...
@define('NAVLINK_AMOUNT', 'ナビゲーション バーのリンク数の入力 (and save this form)');
