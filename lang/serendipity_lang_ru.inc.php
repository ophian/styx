...
@define('HERE_YOU_CAN_ENTER_BLAHBLAH', 'Здесь вы можете настроить размер изображения. Если вы хотите изменить размер изображения пропорционально, просто введите значение в одно из двух полей и нажмите клавишу TAB или кнопку мыши — новый размер будет рассчитан автоматически, чтобы пропорции изображения не были нарушены. <p><b>ВНИМАНИЕ:</b> это не высокотехнологичный инструмент для изменения размера изображений, точно настроенный под потребности конкретного изображения. <br>Каждое изменение масштаба приводит к более или менее значительной потере качества изображения по сравнению с исходным файлом. И эта потеря увеличивается с каждым последующим изменением масштаба!</p>');
...
@define('PGSQL_REQUIRED', 'Вы должны иметь поддержку PostgreSQL чтобы выполнить это действие.');
@define('COULDNT_CONNECT', 'Не удалось подключиться к базе данных MySQL : %s.');
@define('PGSQL_COULDNT_CONNECT', 'Не удалось подключиться к базе данных PostgreSQL: %s.');
...
@define('DO_MARKUP_DESCRIPTION', 'Применять (плагин) преобразования разметки к тексту (смайлы, bbcode, разметка ярлыков s9y, markdown и т.д.). Если опция отключена, содержимое отображается 1:1, и любое HTML-форматирование сохраняется. Если эта опция активирована, другие плагины могут изменять содержимое самородка.');
...
@define('INSTALL_DBPREFIX_DESC', 'Префикс таблиц в вашей базе даынных, например styx_');
...
@define('INSTALL_WYSIWYG', 'Используйте редактор RichText (WYSIWYG)');
@define('INSTALL_WYSIWYG_DESC', 'Этот редактор будет использоваться в обычных текстовых полях, таких как формы ввода, статические страницы, комментарии (которые имеют дополнительную глобальную настройку) и некоторые другие места плагинов; Он хранит HTML-разметку напрямую.');
...
@define('DELETE_DIRECTORY_DESC', 'Вы собираетесь удалить каталог<sup>(¹)</sup>, который может содержать медиафайлы и элементы, уже использованные в ваших записях. Убедитесь в этом!<br><sup>[¹]</sup> Включая скрытый подкаталог с файлами вариаций (WebP/AVIF).');
@define('FORCE_DELETE', 'Удалите ВСЕ файлы в этой директории, включая поддиректории и те файлы, которые еще не известны MediaLibrary');
...
@define('CONVERT_HTMLENTITIES', 'Пытаться автоматически конвертировать HTML?'); // Translate again
...
@define('BACKEND_TITLE_FOR_NUGGET', 'Здесь вы можете определить короткую пользовательскую строку, которая отображается на экране Plugin Configuration вместе с описанием плагина. Если у вас несколько уложенных в стек плагинов или HTML самородков с пустым заголовком, это поможет отличить плагины от других.');
...
@define('INSTALL_PERMALINK_AUTHOR_DESC', 'Здесь вы можете определить структуру относительных адресов, начинающихся с вашего основного адреса, по которому будут доступны записи от отдельных авторов. Вы можете использовать переменные %id%, %realname%, %username%, %email% и любые другие символы. ');
...
@define('ERROR_XSRF', 'Ваш браузер не отправил корректную строку HTTP-Referrer. Для реферера это может быть вызвано либо неправильно настроенным браузером/прокси, либо направленной на вас подделкой межсайтового запроса (XSRF). В противном случае это сообщение может свидетельствовать только о том, что ваша вкладка была открыта слишком долго и срок действия вашего текущего токена редактирования истек. Запрошенное вами действие не может быть выполнено.');
