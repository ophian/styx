...
@define('ORIGINAL_SIZE', 'Größe des Ausgangsbildes: <i>%sx%s</i> Pixel');
@define('HERE_YOU_CAN_ENTER_BLAHBLAH', 'Hier können Sie die Bildgröße ändern. Um die Proportionen beizubehalten, einfach einen der beiden Werte eingeben, anschließend die TAB-Taste oder die Maus drücken, und Sie erhalten die Voreinstellung mit den korrekten Proportionen.<p><b>ACHTUNG:</b> Dies ist kein "HIGH END" Skalierungs-Werkzeug, abgestimmt auf die spezifischen Bedürfnisse eines jeden Bildes.<br>Verglichen mit dem Ausgangsbild bringt jede Skalierung zwangsläufig einen kleineren oder größeren Qualitätsverlust mit sich. Und dies potenziert sich mit jeder weiteren Skalierung !</p>');
@define('SCALE_VARIATION_FORMAT', 'Da wir annehmen, dass Sie das Proportionsverhältnis bei einer Skalierung (empfohlenermaßen) <b>beibehalten</b>, wird eine veränderte Bild "Format" Variation ["webp/avif"] nur vom Ausgangsbild erstellt und <b>KEINE</b> Änderung der kleineren Vorschaubilder vorgenommen, da diese sich, bei bestimmten Bildeigenschaften und Formaten, in der Dateigröße nur unnötig aufblähen und damit kontraproduktiv wirken können. So gesehen, verbleiben alle kleineren Vorschaubilder in der Dimension, die zur Zeit des Uploads des Originalbildes gültig war, da sich das proportionale Verhältnis nicht geändert hat.<p>Wenn Sie wirklich eine Skalierung mit einer <b>anderen</b> Proportion <b>und</b> einer zusätzlichen Änderung des kleineren Vorschaubildes benötigen, müssen Sie die "<em>Proportionen beibehalten</em>" Checkbox deaktivieren und dafür die "<em>...Vorschau Variation</em>" Checkbox <b>aktivieren</b>. (Vorzugsweise bevor Sie die Zahlen ändern.)</p>');
...
@define('INSTALL_DBPASS_DESC', 'Das Passwort, das dem oben genannten Benutzernamen entspricht. Wenn bereits gesetzt, existiert es nur in der Zugangs "serendipity_config_local.inc" Datei, oder kann hier neu gesetzt werden.');
...
@define('USERCONF_LANG_DESC', 'Die Sprache ihrer persönlichen Präferenz');
...
@define('USER_PERMISSION_NOTIFIER_DRAFT_MODE', 'Ihnen wurde noch nicht das Recht gewährt ihre Einträge direkt zu veröffentlichen. Bis genügend Vertrauen aufgebaut ist, informieren Sie ihren zugeordneten Chefredakteur / Administrator darüber, dass ihr Eintrag zur Abnahme und Veröffentlichung fertig bereit steht.');
@define('USER_PERMISSION_NOTIFIER_WRITE_MODE', 'Ihnen wurden (noch) keine Schreibrechte auf diesem Blog gewährt. Bis genügend Vertrauen aufgebaut ist, informieren Sie ihren zugeordneten Chefredakteur / Administrator darüber, dass Sie einen Eintrag veröffentlichen möchten und senden diesen zur Abnahme und Veröffentlichung gleich mit.');
...
@define('ERROR_XSRF', 'Ihr Browser hat keinen gültigen HTTP-Referrer übermittelt. Dies kann für den Referrer entweder daher kommen, dass Ihr Browser/Proxy nicht korrekt konfiguriert ist, oder dass Sie Opfer einer "Cross Site Request Forgery (XSRF)" waren, mit der man Sie zu ungewollten Änderungen zwingen wollte. Ansonsten kann dies auch nur darauf hindeuten, dass ihr Tab zu lange offen war und ihr augenblickliches Bearbeitungstoken abgelaufen ist. Die angeforderte Aktion konnte daher nicht durchgeführt werden.');
...
@define('MEDIA_UPLOAD_MAXWIDTH_PORTRAIT', 'Maximale (opt.) Breite für Bilder im Hoch-Format');
@define('MEDIA_UPLOAD_MAXWIDTH_PORTRAIT_DESC', 'Geben Sie (optional) die maximal erlaubte Breite für hochzuladende Hochformat-Bilder in Pixeln an. Wichtig, wenn Sie die “Größe für Upload automatisch anpassen” Option aktivieren und generell nur Bilder einer damit festgelegten Basis-Breite unterschiedlich für Portrait und Landscape Formate generieren lassen wollen. Dies findet nur Anwendung, wenn die Optionen “Längste Seite” und genannte “Größe ... automatisch anpassen” gegeben sind. Diese Angabe entbindet nicht der folgenden Festlegung einer bestimmten maximalen Höhenbegrenzung.');
...
@define('INSTALL_OFFSET_ON_SERVER_TIME_DESC', 'Eintragszeiten nach Server-Zeitzone eintragen oder nicht. "Ja" wählt die Server-Zeitzone als Basis, "Nein" wählt GMT.');
...
@define('SIMPLE_FILTERS_DESC', 'Wenn aktiviert, werden Suchformulare und Filteroptionen auf die notwendigen Optionen reduziert. Ist diese Option deaktiviert, werden alle erweiterten Filter- und Bearbeitungsoptionen angezeigt, zB. in der "Einträge bearbeiten" Liste oder in der "Mediathek", die den aktuellen User-Berechtigungen entsprechen.');
...
@define('CLEAR_SELECTION', 'Ganze Auswahl zurücksetzen');
@define('CLEAR_FIELD', 'Textfeld leeren');
...
@define('USERCONF_BACKENDPOPUP_GRANULAR', 'Anzeigemodus-Überschreibungen (Eingebettet vs. Modal)');
@define('USERCONF_BACKENDPOPUP_GRANULAR_DESC', 'Bestimmt, welche Backend-Elemente direkt in die Editor-Ansicht integriert (eingebettet) angezeigt werden, anstatt als Modal-Dialog zu öffnen. Standardmäßig werden „categories,tags,links“ inline unterstützt.');
@define('START_UPDATE', 'Starte Update ...');
@define('UPDATE_ALL', 'Update Alle');
@define('JS_FAILURE', 'Die Serendipity JavaScript-Datei konnte nicht geladen werden. Dies kann aufgrund von PHP- oder Plugin-Fehlern, oder sogar von einem fehlerhaften Browser Pufferspeicher (cache) geschehen. Um die exakte Fehlermeldung anzusehen, öffnen Sie bitte diesen Link <a href="%1$s">%1$s</a> in ihrem Browser. Ein solcher Fehler kann vielfältige Ursachen haben, ist zumeist aber nur eine Stapel (Fehler-) Ausgabe zur Unzeit und verhindert hier die korrekte Ausführung der Javascript Datei.');
...
@define('USE_CACHE_DESC', 'Wenn diese Option aktiviert ist, erlaubt Serendipity einen internen Puffer (cache) einzuschalten, um Blog-Einträge bzw einzelne Blöcke im Frontend zwischenzuspeichern, solange sich an den dargestellten Inhalten nichts ändert. Dadurch verringert sich die Last auf Servern mit mittlerem bis hohem Verkehr und verbessert die Ladezeit. Sie können mittels aktivierten <a href="https://www.php.net/manual/en/intro.apcu.php" target="_blank" rel="noopener">APCu</a> diesen Cache komplett im RAM des Servers halten oder mit PHPs bundled <a href="https://www.php.net/manual/en/book.opcache.php" target="_blank" rel="noopener">OpCache</a> dies im Serendipity Filesystem unter <code>templates_c/simple_cache/</code> geschehen lassen. Die Standardzeit bis zur Erneuerung beträgt eine Stunde. Unabhängig von dieser Option nutzen einzelne Plugins diesen Cache bereits für ihre Seitenleisten (Block-) Caches, wobei der Zeitraum der Vorhaltung variieren kann (zB. 1 Tag).<br><br>Im Unterschied zum "Entryproperties" Cache haben wir es also hier mit einem Ausgabe Cache zu tun, während jener nur die Markup Umwandlung in HTML zwischenspeichert, was nur mit aktivierten Markup-Plugins oder auf einem älteren System mit gewechselten Eingabestandards Sinn ergibt.');
...
@define('HIDE_SUBDIR_FILES', 'Exklusive Dateien von Unterverzeichnissen');
...
@define('HTML_COMMENTS_DESC', 'Schaltet HTML und den graphischen RichText Editor für Kommentare frei. Dies geschieht als unabhängige Option im Kommentarform der Frontend-Blogeinträge, aber nur zusätzlich im Backend für die Kommentarbearbeitung und -anzeige. Sie müssen dafür also die Rich-Text-Editor Option in den "Eigenen Einstellungen" aktiviert haben. Dann werden zusätzlich "tag"-beschränkte HTML-Kommentare und "pre/code" HTML-Auszeichnungs-Bestandteile zugelassen, die in Backend- und Frontend-Seiten als HTML verarbeitet, aber vom Editor nur im Backend bearbeitet werden. Bitte beachten Sie: Diese Option gibt auch alte Kommentare frei, um ihren eventuellen Markup Inhalt als HTML anzuzeigen. Überprüfen Sie diese also besser vor der Erlaubnis (!), damit Sie keine versehentlich genehmigten, aber unsichere (javascript oder links, etc) Kommentar Inhalte in Ihrer Datenbank gespeichert haben. Ansonsten eignen sich Rich-Text-Editor-Kommentare auch um lästige Bot-Programm-Spammer loszuwerden. Diese Option funktioniert im Frontend z.Z. nur vollständig mit dem Standard "Pure" Theme und einigen wenigen anderen und ist deshalb auch nur dort zu empfehlen!');
