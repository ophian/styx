...
@define('HERE_YOU_CAN_ENTER_BLAHBLAH', '여기서 이미지 크기를 조정할 수 있습니다. 비율을 유지하며 크기를 조정하려면 두 상자 중 하나에 값을 입력한 후 탭 키나 마우스 키를 누르세요. 이미지의 비율이 깨지지 않도록 새 크기가 자동으로 계산됩니다. <p><b>주의사항:</b> 본 도구는 특정 이미지 요구사항에 맞춰 정밀하게 조정된 고급 이미지 편집기용 크기 조정 도구가 아닙니다.<br>모든 크기 조정은 원본 입력 파일에 비해 어느 정도 이미지 품질 저하를 동반합니다. 그리고 이는 추가적인 크기 조정마다 더욱 심해집니다!</p>');
...
@define('CANT_EXECUTE_BINARY', '%s 바이너리 경로를 실행할 수 없습니다 (확장 모듈 사용 시 필수 사항 아님)');
...
@define('USERCONF_BACKENDPOPUP_GRANULAR_DESC', 'Select which backend elements should be integrated directly into the editor view (embedded) instead of opening in an overlay modal dialog. Default options with native inline support are "categories,tags,links".');@define('START_UPDATE', '업데이트 시작...');
