...
@define('THUMB_CREATED_DONE', '缩图建立，<br>%s 完成！'); // ADD: and of all variations
...
@define('HERE_YOU_CAN_ENTER_BLAHBLAH', '在此处可调整图像尺寸。若需按比例调整图像大小，只需在两个输入框中的任意一个输入数值，然后按下TAB键或鼠标键——系统将自动计算新尺寸，确保图像比例不变。<p><b>请注意：</b>本工具并非高端图像编辑器的专业缩放功能，无法针对特定图像需求进行精细调整。<br>每次缩放都会导致图像质量相较原始文件不同程度下降，且随缩放次数增加而加剧！</p>');
...
@define('CANT_EXECUTE_BINARY', '无法执行 %s 二进制路径（使用扩展模块时非必需）');
...
@define('SERENDIPITY_UPGRADER_YOU_HAVE_IGNORED', '你跳过了更新任务, 请确定数据库已安装完成, 和其它的任务安装无误！ 在完成合理的回退工作后，只需重置 serendipity_config_local.inc 文件中的版本，并通过后台页面再次运行升级页面，即可自动完成回退。');
...
@define('DELETE_DIRECTORY_DESC', '您将删除一个目录<sup>(¹)</sup>，其中可能包含媒体文件和已在您的条目中使用的项目。请务必！<br><sup>[¹]</sup> 包括包含 (WebP/AVIF) 变体文件的隐藏子目录。');
...
@define('BACKEND_TITLE_FOR_NUGGET', '在这里，您可以定义一个简短的自定义字符串，它与插件的描述一起显示在插件配置屏幕中。如果您有多个堆叠的插件或HTML掘金块的空标题，这有助于区分不同的插件。');
...
@define('ERROR_XSRF', '您的浏览器没有发送有效的 HTTP-Referrer 字符串。对于推荐人来说，这可能是由于浏览器/代理服务器配置错误或针对您的跨站请求伪造（XSRF）造成的。否则，此消息只能说明您的标签页打开时间过长，您当前的编辑令牌已过期。您请求的操作无法完成。');
