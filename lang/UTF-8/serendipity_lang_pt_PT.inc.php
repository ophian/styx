...
@define('IMAGESYNC_WARNING', 'ATENÇÃO:<br>Isso pode demorar muito tempo se existirem muitas imagens sem miniaturas.');
...
@define('MEDIA_GALLERY_SELECTION', 'This particular selection for media galleries only shows directory images of the same level. It does not contain a statement to also display the images of sub folders, as you might be used to. The number of possible preview images that can be displayed at the same time is limited to 48 items. Restructure your media library accordingly!<br>This media gallery directory selection only shows thumbnails (optionally configurable linking to the big picture). If your preview images do not meet the standard of 400px, and are much smaller than the defined gallery format of 260px, it is possible that you get into display problems in this selection as well as afterwards in the frontend entry.<br>The order of displayed gallery items is not configurable here and can only be changed within the displayed source code of your add-on editor window. If you there use the WYSIWYG-Editor please do <strong>not</strong> use the internal <em>drag & drop</em> feature of the WYSIWYG-mode (editor) window, since that will destroy or at least mess up the gallery selection for your entry.');
