...
@define('USER_ACTION', 'Benutzeraktion');
@define('MANDATORY', '[ zwingend erforderlich ]');
...
@define('SERENDIPITY_UPGRADER_YOU_HAVE_IGNORED', 'Sie haben den automatischen Aktualisierungsvorgang ignoriert. Bitte stellen Sie sicher, dass die Datenbank anhand möglicher SQL-Dateien aktualisiert und die geplanten Funktionen aufgerufen werden! Nachdem Sie Ihre begründete Arbeit für das Zurückhalten beendet haben, kann dies automatisch geschehen, indem Sie einfach die Version in Ihrer serendipity_config_local.inc Datei zurücksetzen und diese Upgrade-Seite erneut über Ihre Backend-Seite aufrufen.');
...
Setzen Sie ein Häkchen bei den entsprechenden Dateien und wählen hier den Ort zum Verschieben aus.
...
@define('DIRECTORY_INFO_DESC', 'Verzeichnisse spiegeln ihren physikalischen Verzeichnis Namen. Wenn Sie Verzeichnisse ändern oder verschieben wollen, welche bereits Elemente enthalten, haben Sie zwei Möglichkeiten: Entweder erstellen Sie vorher das benötigte Verzeichnis oder Unterverzeichnis und verschieben dann die Elemente über die Mediathek in das neue Verzeichnis und löschen danach hier das leere alte Verzeichnis, oder Sie ändern den Namen des alten Verzeichnisses einfach im jeweiligen Bearbeitungsmenu des Verzeichnisses selbst und benennen es um (zB. von A/B/ zu A/C/). Dadurch werden alle Verzeichnisse und Elemente verschoben bzw das bestehende Verzeichnis umbenannt und verweisende Blog-Einträge entsprechend geändert.');
...
@define('UPDATE_NOTIFICATION_URL_DESC', 'Dies ist Styx! Bitte nur ändern, wenn Sie eine andere Ankündigungs-Datei für benutzerdefinierte System Downloads, in Kombination mit dem Serendipity Autoupdate Plugin, erstellen wollen. Eine eigene Intranet URL zur Datei würde dann zum Beispiel so lauten: "https://localhost/git/you/styx/docs/RELEASE". Eine solche Datei enthält die Versionsnummern der aktuell freigegebenen (stable/beta) Veröffentlichung für das Autoupdate Plugin; zB. "stable:5.3.0". Die Styx Repository default URL ist: "https://raw.githubusercontent.com/ophian/styx/master/docs/RELEASE"');
...
@define('DASHBOARD_SYSTEM_TICKER', 'Remote-Systembenachrichtigungen');
@define('DASHBOARD_SYSTEM_TICKER_DESC', 'Erlaubt versteckbare Remote-Systembenachrichtigungen in der Backend (dashboard) Übersichtsseite.');
...
@define('MAINTENANCE_COLUMN_SORTNOTE', 'Diese Seite verwendet eine variable Blockspaltensortierung; sie kann die Reihenfolge der Spaltenblöcke dynamisch verändern, wenn Sie den Blockinhalt erweitern.');
...
@define('ENABLEAVIF', 'Nutzung von AVIF Variationen (PHP 8.1+) aktivieren');
@define('ENABLEAVIF_DESC', 'Image AVIF Variationen können sehr ressourcenintensiv sein, da viel Ram und CPU/GPU Kerne benötigt werden um Bilder in das AV1 Format zu kodieren. Massenuploads und Massenkonvertierungen (siehe "Wartung") sind daher nicht zu empfehlen. Erlernen Sie den Umgang an ein paar Beispielen, bevor Sie die Nutzung verbleibend erlauben. PHP 8.1 fehlt noch eine entscheidende Funktion, um Größenangaben mit den üblichen Methoden aus AVIF Dateien auszulesen. Dies bedingt vorläufig auch, dass die Bild-Funktionen der Mediathek "Dateiausmaße ändern" und "Bild um 90° drehen" für alle Formate nicht verwendet werden können, wenn Sie AVIF verwenden, da ja jede dieser Aktionen das Originalbild, sowie dessen Variationen betrifft. Das Problem ist mit PHP 8.2 gelöst, welches die fehlende Funktion nachliefert.');
