...
@define('THUMB_CREATED_DONE', 'Előnézeti %s kép létrehozva.<br>Kész.'); // ADD: and of all variations
...
@define('HERE_YOU_CAN_ENTER_BLAHBLAH', 'Itt állíthatja be a kép méretét. Ha a képet arányosan szeretné átméretezni, csak írjon be egy értéket a két mező egyikébe, és nyomja meg a TAB vagy az egér gombot – a program automatikusan kiszámítja az új méretet, így a kép arányai nem romlanak el. <p><b>FIGYELEM:</b> Ez nem egy csúcskategóriás képszerkesztő eszköz, amelyet egy adott kép igényeihez igazítottak. <br>Minden méretezésnél a képminőség többé-kevésbé romlik az eredeti bemeneti fájlhoz képest. És ez minden további méretezéssel tovább romlik!</p>');
@define('SCALE_VARIATION_FORMAT', 'Since we assume you <b>keep</b> the files proportion (recommended), a scaled image "format" variation ["webp/avif"] change will be applied to the Origin files variation only and <b>NOT</b> to the thumbnails of all formats, which - by certain image property conditions - would probably blow up its filesize and so have counter-productive effects. From this perspective, the smaller preview images remain in the dimensions that were valid at the time the original image was uploaded, as the proportional ratio has not changed.<p>If you really need an image scale with an <b>other</b> proportion <b>and</b> an additional changed thumb dimension size, de-activate the "<em>..keep proportion</em>" checkbox and activate the "<em>..thumb variation</em>" checkbox instead. (Preferably before changing the numbers.)</p>');
...
@define('CANT_EXECUTE_BINARY', 'A %s bináris útvonal nem hajtható végre (nem kötelező a kiterjesztés modul használata esetén)');
...
@define('DELETE_THUMBNAIL', 'A következő előnézeti kép törlése: %s');
@define('DELETE_FILE', 'A következő fájlt töröltem: %s');
@define('ABOUT_TO_DELETE_FILE', 'A következő fájl törlésére készülsz: <b>%s</b><br>Ha van olyan bejegyzés ahol ezt használod, akkor így törött hivatkozások vagy hiányzó képek lesznek<br>Biztosan folytatod?');
...
@define('WELCOME_TO_INSTALLATION', 'Üdvözöllek a Serendipity Styx telepítőjében');
...
@define('YELLOW', 'sárga');
@define('GREEN', 'zöld');
@define('PRE_INSTALLATION_REPORT', 'Serendipity Styx v.%s telepítési ellenőrzés jelentés');
...
@define('FORCE_DELETE', 'A könyvtárban lévő MINDEN fájl törlése, beleértve az alkönyvtárakat és a MediaLibrary által (még) nem ismert fájlokat is');
...
@define('PLUGIN_GROUP_BACKEND_MAINTAIN', 'Háttér: Maintenance');
@define('PLUGIN_GROUP_BACKEND_DASHBOARD', 'Háttér: Dashboard');
...
@define('PERMISSION_ADMINIMPORT', 'adminImport: Bejegyzések importálása');
@define('PERMISSION_ADMINCATEGORIES', 'adminCategories: Kategóriák adminisztrálása');
@define('PERMISSION_ADMINCATEGORIESMAINTAINOTHERS', 'adminCategoriesMaintainOthers: Más felhasználók kategóriáinak adminisztrálása');
@define('PERMISSION_ADMINCATEGORIESDELETE', 'adminCategoriesDelete: Kategóriák törlése');
@define('PERMISSION_ADMINUSERS', 'adminUsers: Felhasználók adminisztrálása');
@define('PERMISSION_ADMINUSERSDELETE', 'adminUsersDelete: Felhasználók törlése');
@define('PERMISSION_ADMINUSERSEDITUSERLEVEL', 'adminUsersEditUserlevel: Felhasználói szint megváltoztatása');
@define('PERMISSION_ADMINUSERSMAINTAINSAME', 'adminUsersMaintainSame: A csoportodban lévő felhasználók adminisztrálása');
@define('PERMISSION_ADMINUSERSMAINTAINOTHERS', 'adminUsersMaintainOthers: A csoportodba nem tartozó felhasználók adminisztrálása');
@define('PERMISSION_ADMINUSERSCREATENEW', 'adminUsersCreateNew: Felhasználók létrehozása');
@define('PERMISSION_ADMINUSERSGROUPS', 'adminUsersGroups: Felhasználói csoportok adminisztrálása');
@define('PERMISSION_ADMINPLUGINS', 'adminPlugins: Modulok adminisztrálása');
@define('PERMISSION_ADMINPLUGINSMAINTAINOTHERS', 'adminPluginsMaintainOthers: Más felhasználók moduljainak adminisztrálása');
@define('PERMISSION_ADMINIMAGES', 'adminImages: Média fájlok adminisztrálása');
@define('PERMISSION_ADMINIMAGESDIRECTORIES', 'adminImagesDirectories: Média könyvtárak adminisztrálása');
@define('PERMISSION_ADMINIMAGESADD', 'adminImagesAdd: Új média fájlok hozzáadása');
@define('PERMISSION_ADMINIMAGESDELETE', 'adminImagesDelete: Média fájlok törlése');
...
@define('PERMISSION_ADMINIMAGESSYNC', 'adminImagesSync: Előnézeti képek szinkronizálása');
...
@define('ERROR_XSRF', 'A böngészője nem küldött érvényes HTTP-Referrer karakterláncot. A Referrer esetében ezt vagy egy rosszul beállított böngésző/proxy, vagy egy Önt célzó Cross Site Request Forgery (XSRF) okozhatta. Egyébként ez az üzenet csak azt jelezheti, hogy a lapja túl sokáig volt nyitva, és az aktuális szerkesztési tokenje lejárt. Az Ön által kért műveletet nem sikerült végrehajtani.');
...
@define('STYLE_OPTIONS_NONE', 'Ennek a témának/stílusnak nincsenek speciális opciói. A sablon opcióihoz olvasd el a dokumentációt a következő oldalon "https://ophian.github.io/hc/en/templating.html#docs-theme-options" a "Configuration of Theme options" résznél.');
...
@define('FEED_CUSTOM_DESC', 'If set, a custom feed URL can be set to forward Feedreaders to a specific URL. Was useful for statistical analyzers like Feedburner in the past, in which case you would have entered your Feedburner-URL here.');
...
@define('CONF_USE_AUTOSAVE_DESC', 'When enabled, the text written to Blog entries will be saved into the browser\'s session storage periodically. If the browser crashes during writing, at next time a new entry is created, the text will be restored from this autosave. This may have side-effects, so keep on regard. This only is for the described rare case of browser crashes or ending sessions during writing, not for DRAFT or long thinking entry development.');
...
@define('USERCONF_BACKENDPOPUP_GRANULAR', 'Display mode overrides (Embedded vs. Modal)');
@define('USERCONF_BACKENDPOPUP_GRANULAR_DESC', 'Select which backend elements should be integrated directly into the editor view (embedded) instead of opening in an overlay modal dialog. Default options with native inline support are "categories,tags,links".');
...
@define('USE_CACHE_DESC', 'If this option is activated, Serendipity allows you to activate an internal buffer (cache) to temporarily store blog entries or individual blocks on the frontend as long as the displayed content does not change. This reduces the load on servers with medium to high traffic and improves the loading time. You can keep this cache completely in the server\'s RAM by activating <a href="https://www.php.net/manual/en/intro.apcu.php" target="_blank" rel="noopener">APCu</a> or use PHP\'s bundled <a href="https://www.php.net/manual/en/book.opcache.php" target="_blank" rel="noopener">OpCache</a> to do this in the Serendipity file system under <code>templates_c/simple_cache/</code>. The standard time until renewal is one hour. Regardless of this option, individual plugins already use this cache for their sidebar (block) caches, whereby the retention period can vary (e.g. 1 day).<br><br>In contrast to the “Entryproperties” cache, we are dealing here with a real output cache, while the latter only caches the markup conversion to HTML, which only makes sense with activated markup plugins or on an older system with changed input standards.');
...
@define('HTML_COMMENTS_DESC', 'Allows the use of RichText Editor for comments; Independent in frontend, additional in backend. If the RichText Editor option in personal preferences is set true, you additionally allow tag-restricted HTML comments and "pre/code" tag parts displayed in backend and frontend pages, but edited by Editor in backend only. Keep in mind: This options liberates old comments to display their content. So better check them up before (!), that you don\'t have accidentally approved spoofed content in your database stored comments. Otherwise, RichText Editor comments are also suitable to get rid of annoying bot program spammers. For the frontend, this option currently only works completely with the standard "Pure" theme and a few others and is therefore only recommended there!');
