...
@define('HERE_YOU_CAN_ENTER_BLAHBLAH', 'در اینجا می‌توانید اندازه تصویر را تنظیم کنید. اگر می‌خواهید تصویر را به صورت متناسب تغییر اندازه دهید، کافی است یک عدد را در یکی از دو کادر وارد کرده و کلید TAB یا ماوس را فشار دهید -- این کار به طور خودکار اندازه جدید را محاسبه می‌کند تا نسبت‌های تصویر به هم نریزد. <b>لطفاً توجه داشته باشید:</b> این یک ابزار تغییر اندازه با کیفیت بالا و تنظیم‌شده برای نیاز یک تصویر خاص نیست. هر بار تغییر اندازه، به میزان بیشتر یا کمتری باعث افت کیفیت تصویر نسبت به فایل ورودی اصلی می‌شود. و این افت کیفیت با هر بار تغییر اندازه بعدی بیشتر می‌شود!');
...
@define('DO_MARKUP_DESCRIPTION', 'تغییرات نشانه گذاری (پلاگین) را روی متن اعمال کنید (شککک ها، bbcode، نشانه گذاری های میانبر s9y، علامت گذاری، و غیره). اگر غیرفعال باشد، محتوا 1:1 رندر می شود و هرگونه قالب بندی HTML حفظ می شود. اگر این گزینه فعال باشد، افزونه های دیگر می توانند محتویات قطعه را تغییر دهند.');
...
@define('DELETE_DIRECTORY_DESC', 'شما در شرف حذف دایرکتوری <sup>(¹)</sup> هستید که ممکن است حاوی فایل های رسانه ای و مواردی باشد که قبلاً در ورودی های شما استفاده شده است. مطمئن باشید!<br><sup>[¹]</sup> از جمله زیرمجموعه پنهان با فایل‌های تنوع (WebP/AVIF).');
@define('FORCE_DELETE', 'تمام فایل‌های این فهرست، از جمله زیرمجموعه‌ها و فایل‌هایی که (هنوز) توسط MediaLibrary شناخته نشده‌اند را حذف کنید');
...
@define('ERROR_XSRF', 'مرورگر شما یک رشته HTTP-Referrer معتبر ارسال نکرده است. برای Referrer، این مشکل ممکن است یا به دلیل پیکربندی نادرست مرورگر/پروکسی یا به دلیل جعل درخواست میان‌سایتی (XSRF) که شما را هدف قرار داده است، رخ داده باشد. رخ داده باشد. در غیر این صورت، این پیام فقط می‌تواند نشان دهد که برگه شما مدت زیادی باز بوده و توکن ویرایش فعلی شما منقضی شده است. عملی که درخواست کرده‌اید قابل انجام نیست.');
...
@define('STYLE_OPTIONS_NONE', 'این پوسته تنظیماتی ندارد. برای تغییر تنظیمات این پوسته، راهنمای تکنیکی مربوط به این بخش رو در سایت "https://ophian.github.io/hc/en/templating.html#docs-theme-options" مطالعه کنید.');
...
@define('MEDIA_DYN_RESIZE_DESC', 'در صورت فعال بودن، ابعاد ارسال شده را برای ابعاد تصویر در نظر گرفته می شود. نتیجهء کار نکهداری می شود، پس شما می توانید آرشیو کامل تری ایجاد کنید.'); //check translation
