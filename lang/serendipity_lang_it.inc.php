...
@define('HERE_YOU_CAN_ENTER_BLAHBLAH', 'Qui è possibile regolare le dimensioni dell\'immagine. Se si desidera ridimensionare l\'immagine in modo proporzionale, è sufficiente inserire un valore in una delle due caselle e premere il tasto TAB o il tasto del mouse: il programma calcolerà automaticamente le nuove dimensioni in modo che le proporzioni dell\'immagine non vengano alterate. <p><b>NOTA BENE:</b> questo non è uno strumento di ridimensionamento immagini di fascia alta, ottimizzato per le esigenze di un\'immagine specifica. Ogni ridimensionamento comporta una perdita più o meno significativa della qualità dell\'immagine rispetto al file originale. E questa perdita aumenta con ogni ulteriore ridimensionamento!</p>');
...
@define('CANT_EXECUTE_BINARY', 'Impossibile eseguire il percorso binario %s (non obbligatorio quando si utilizza il modulo di estensione)');
...
@define('INSTALL_DBPASS_DESC', 'La password dell\'utente indicato. Se già impostato, esiste solo nel file di accesso "serendipity_config_local.inc", oppure può essere impostato qui.');
...
@define('INSTALL_WYSIWYG', 'Utilizzare l\'editor RichText (WYSIWYG)');
@define('INSTALL_WYSIWYG_DESC', 'Questo editor sarà usato nei campi textarea più comuni, come i moduli di inserimento, le pagine statiche, i commenti (che hanno un\'opzione globale aggiuntiva) e alcuni altri luoghi del plugin; memorizza direttamente il markup HTML.');
...
@define('INSTALL_IMAGEMAGICK_DESC', 'Hai installato la libreria binaria ImageMagick o il modulo Imagick e desideri utilizzarli per ridimensionare le immagini?');
...
@define('ERROR_XSRF', 'Il browser non ha inviato una stringa HTTP-Referrer valida. Per quanto riguarda il referente, ciò può essere stato causato da un browser/proxy mal configurato o da una Cross Site Request Forgery (XSRF) rivolta all\'utente. Altrimenti, questo messaggio può solo indicare che la scheda è rimasta aperta troppo a lungo e il token di modifica corrente è scaduto. Non è stato possibile completare l\'azione richiesta.');
