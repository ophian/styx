...
@define('HERE_YOU_CAN_ENTER_BLAHBLAH', 'Aqui, pode ajustar o tamanho da imagem. Se quiser redimensionar a imagem proporcionalmente, basta introduzir um valor numa das duas caixas e pressionar a tecla TAB ou o botão do rato — o novo tamanho será calculado automaticamente para que as proporções da imagem não sejam alteradas. <p><b>ATENÇÃO:</b> Esta não é uma ferramenta de redimensionamento de imagens de alta qualidade, ajustada para as necessidades de uma imagem específica.<br>Cada escala resulta numa perda mais ou menos acentuada da qualidade da imagem em comparação com o ficheiro original. E esta perda aumenta a cada nova escala!</p>');
...
@define('INSTALL_DBPASS_DESC', 'A senha correspondente ao utilizador acima. Se já estiver definido, só existe no ficheiro de acesso "serendipity_config_local.inc", ou pode ser definido aqui.');
...
@define('INSTALL_IMAGEMAGICK_DESC', 'Tem a biblioteca binária ImageMagick ou o módulo Imagick instalado e quer usá-lo para redimensionar imagens?');
...
@define('ERROR_XSRF', 'O seu browser não enviou uma cadeia de caracteres HTTP-Referrer válida. Para o Referenciador, isto pode ter sido causado por um navegador/proxy mal configurado ou por uma falsificação de pedidos entre sítios (XSRF) dirigida ao utilizador. Caso contrário, esta mensagem pode apenas indicar que o seu separador está aberto há demasiado tempo e que o seu token de edição atual expirou. A ação solicitada não pôde ser concluída.');
