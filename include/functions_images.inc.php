...
function serendipity_passToCMD(?string $type = null, string $source = '', string $target = '', iterable $args = array()) : string|iterable|bool {
    ...
    // Yes, there were versions which were buggy, but since 6.9.x and 7.1.0-43 you are quite save awaiting this as standardized set.
    ...
    // Type [image/$mime] is used when already known (like come from DB and the scale, rotate etc.
        ...
        if ($frames > 1) {
            ...
            // STATIC GIF
            $cmd = "\"{$args[0]}\" \"$source\" {$do} -strip \"$target\"";
            $dbg .= "static GIF workflow \n";
        }
        $dbg .= "source toThumb from $type [ $cmd ]\n";
...
function serendipity_scaleImg(int $id, int $width, int $height, bool $scaleThumbVariation = false) : bool|string|null {
    ...
    if ($debug) {
        $logtag = 'ML_SERENDIPITY_SCALEIMG::';
        $serendipity['logger']->debug("\n" . str_repeat(" <<< ", 10) . "DEBUG START ML serendipity_scaleImg() SEPARATOR" . str_repeat(" <<< ", 10) . "\n");
    }
    ...
    if (file_exists($oavif)) {
        ...
        if (($_width == 0 && $_height == 0 && $_type = 19) || filesize($oavif) <= 252) {
    ...
    if ($serendipity['magick'] !== true) {
        ...
        if ($result[0] != 0) {
            ...
            if ($debug) { $serendipity['logger']->debug("ImageMagick ({$crtby}) Scale File command: {$result[2]}, with {$width}x{$height}{$bang} dimension."); }
            ...
            if (file_exists($owebp)) {
                ...
                if (is_array($reswebp) && $reswebp[0] != 0) {
                    ...
                    if ($debug) { $serendipity['logger']->debug("ImageMagick ({$crtby}) Scale WebP File command: {$reswebp[2]}, with {$width}x{$height}{$bang} dimension."); }
                    if ($scaleThumbVariation && file_exists($owebpTH)) {
                        // if particularly wished, (silently) force scale Thumb Variation too
                        ...
                        if (is_array($resTH) && $resTH[0] == 0 && $debug) {
                            $serendipity['logger']->debug("ImageMagick ({$crtby}) Scale WebP Thumb File command: {$owebpTH}, with {$ntbz[0]}x{$ntbz[1]}{$bang} dimension.");
                        }
                    }
                }
            }
            ...
            if (file_exists($oavif)) {
                ...
                if (is_array($resavif) && $resavif[0] != 0) {
                    ...
                    if ($debug) { $serendipity['logger']->debug("ImageMagick ({$crtby}) Scale AVIF File command: {$resavif[2]}, with {$width}x{$height}{$bang} dimension."); }
                    if ($scaleThumbVariation && file_exists($oavifTH)) {
                        // if particularly wished, (silently) force scale Thumb Variation too
                        #DEV# echo "if particularly wished, (silently) force scale Thumb Variation too ".print_r($fpass,true)."<br>\n<br>\n";
                        // check Imagick module extension vs binary CLI usage
                        ...
                        #DEV# echo print_r($resTH,true)."<br>\n<br>\n";
                        if (is_array($resTH) && $resTH[0] == 0 && $debug) {
                            $serendipity['logger']->debug("ImageMagick ({$crtby}) Scale AVIF Thumb File command: {$oavifTH}, with {$ntbz[0]}x{$ntbz[1]}{$bang} dimension.");
                        }
                    }
                }
            }
...
function serendipity_rotateImg(int $id, int $degrees) : bool {
    ...
    $turn = preg_match('@-@', (string) $degrees) ? 'TURN LFT' : 'TURN RGT';
    ...
    if (file_exists($infile_avif)) {
        ...
        if (($_width == 0 && $_height == 0 && $_type = 19) || filesize($infile_avif) <= 252) {
...
function serendipity_rotateImageGD(string $infilename, string $outfilename, int $degrees) : iterable|false {
    #static $log2err; // enable for error log debugging
    ...
    #if ($log2err === null) trigger_error("GD FIXED degrees $degrees CCW");
...
function serendipity_prepareMedia(iterable &$file, ?string $url = '') : true {
    ...
    $file['popupWidth']   = $file['is_image'] ? $file['dimensions_width'] : 600;
    $file['popupHeight']  = $file['is_image'] ? $file['dimensions_height'] : 500;
