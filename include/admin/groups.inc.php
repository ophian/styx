...
if (isset($_POST['SAVE_NEW']) && serendipity_checkFormToken()) {
    ...
    if (!is_array($_group)) {
        ...
        serendipity_updateGroupConfig((int) $serendipity['POST']['group'], $perms, $serendipity['POST'], false, $_forbidden_plugins, $_forbidden_hooks);
        $data['save_new'] = true;
...
if ($serendipity['GET']['adminAction'] != 'delete') {
    ...
    $data['groups'] = $groups;
    if (!(isset($_POST['NEW']) || $serendipity['GET']['adminAction'] == 'new' || $serendipity['GET']['adminAction'] == 'edit')) {
        $data['start'] = true;
    }
