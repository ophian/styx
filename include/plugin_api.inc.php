...
class serendipity_plugin_api
{
    ...
    static function enum_plugins(string $filter = '*', bool $negate = false, ?string $classname = null, ?string $id = null) : iterable|string|bool
    {
        ...
        if (!empty($classname)) {
            $where[] = " name LIKE '" . serendipity_db_escape_string($classname) . "%' ";
        }
    ...
    static function &get_event_plugins(string|bool $getInstance = false, bool $refresh = false) : iterable|object|string|false
    {
        ...
        $event_plugins = array();
        foreach($plugins AS $plugin_data) {
            if ($event_plugins[$plugin_data['name']]['p'] = &serendipity_plugin_api::load_plugin($plugin_data['name'], (string) $plugin_data['authorid'], $plugin_data['path'])) { // INT to STRING cast for PDO::PostgreSQL
    ...
    static function load_language(string $path) : void
    {
        ...
        $probelang = $path . '/lang_' . $serendipity['lang'] . '.inc.php';
